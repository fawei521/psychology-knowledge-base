...
1. **确认概念点**
   - ai从候选池中选择一个**具体概念点/理论簇**（如「记忆」「浪漫爱情」「认知发展」）。
...
1. **AI 自动选择**：AI 从 `topic-backlog.md` 冲刺池中挑选当前最值得填充的 1 个概念点，优先选择考研权重高、内部关系清晰、当前缺失严重的理论簇。
2. **展示候选（可选）**：若用户此前未指定方向，AI 可展示 1 个主推候选及 1–2 个备选，由用户确认或让 AI 继续；若用户已授权自动推进，AI 直接开始工作流。
3. **开始工作流**：回到本规范第 1 步「确认主题」。
4. **标记完成**：概念点填充并验收完成后，在 `topic-backlog.md` 标记为「已完成」。流程改进、工具 bug、架构决策等通用经验才写入 `PROJECT_LOG.md`。

> **批量填充推进规则**：每完成一个概念簇，AI 输出完成摘要，并自动从冲刺池中选择下一个最合适的概念点继续推进。用户可随时打断、指定方向或喊停。
